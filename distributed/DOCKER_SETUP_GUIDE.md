...
For automated setup, you can use the provided setup scripts for each service:
...
```bash
# 1. Setup Auth Service (no dependencies)
./setup-auth-service.sh

# 2. Setup Tournament Service (no dependencies)
./setup-tournament-service.sh

# 3. Setup Team Service (depends on Tournament Service)
./setup-team-service.sh

# 4. Setup Match Service (no dependencies)
./setup-match-service.sh

# 5. Setup Results Service (depends on all other services)
./setup-results-service.sh
```

### What Each Script Does

Each setup script automatically:
- Waits for the service database to be ready
- Installs Composer dependencies inside the container
- Generates Laravel application key
- Runs database migrations
- Runs database seeders
- Optimizes the application (config:cache, route:cache)
- Provides health check verification

### Script Options

- **Interactive Mode**: Scripts will prompt for confirmation at each step
- **Error Handling**: Scripts include comprehensive error checking and will stop if any step fails
- **Logging**: All operations are logged with clear success/failure messages
- **Dependency Validation**: Scripts check that dependent services are running before proceeding

### Troubleshooting Scripts

If a script fails:
1. Check the specific error message
2. Verify the dependent services are running: `docker-compose ps`
3. Check service logs: `docker-compose logs -f SERVICE_NAME`
4. Re-run the script after fixing the issue
