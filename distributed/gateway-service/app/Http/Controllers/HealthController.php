...
class HealthController extends Controller
{
    ...
    protected function checkExternalServices(): array
    {
        ...
        $serviceUrls = [
            'auth_service' => config('services.auth.url', 'http://localhost:8001'),
            'tournament_service' => config('services.tournament.url', 'http://localhost:8002'),
            'team_service' => config('services.team.url', 'http://localhost:8003'),
            'match_service' => config('services.match.url', 'http://localhost:8004'),
            'results_service' => config('services.results.url', 'http://localhost:8005'),
        ];
        ...
        foreach ($serviceUrls as $serviceName => $url) {
            try {
                ...
                Log::error("Service health check failed: {$serviceName}", ['error' => $e->getMessage()]);
