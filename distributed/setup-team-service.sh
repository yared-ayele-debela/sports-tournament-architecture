...
# Script to setup Team Service
# This script automates the complete setup process for the team-service
# Usage: ./setup-team-service.sh
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Team Service Setup Script${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
# Step 3: Verify and fix team-service .env file
echo -e "${YELLOW}🔍 Checking team-service .env file...${NC}"
if [ -f "team-service/.env" ]; then
    # Check if DB_USERNAME is set correctly
    if grep -q "^DB_USERNAME=user" team-service/.env 2>/dev/null; then
        echo -e "${YELLOW}⚠️  Found DB_USERNAME=user in team-service/.env, updating to root...${NC}"
        # Update to use root user
        sed -i 's/^DB_USERNAME=user$/DB_USERNAME=root/' team-service/.env
        sed -i 's/^DB_PASSWORD=.*$/DB_PASSWORD=rootpassword/' team-service/.env
        echo -e "${GREEN}✅ Updated team-service/.env to use root credentials${NC}"
        echo ""
    elif grep -q "^DB_USERNAME=root" team-service/.env 2>/dev/null; then
        echo -e "${GREEN}✅ team-service/.env is configured correctly${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚠️  DB_USERNAME not found in team-service/.env, ensuring it's set...${NC}"
        # Ensure DB_USERNAME is set
        if ! grep -q "^DB_USERNAME=" team-service/.env 2>/dev/null; then
            echo "DB_USERNAME=root" >> team-service/.env
        fi
        if ! grep -q "^DB_PASSWORD=" team-service/.env 2>/dev/null; then
            echo "DB_PASSWORD=rootpassword" >> team-service/.env
        fi
        echo -e "${GREEN}✅ Added database credentials to team-service/.env${NC}"
        echo ""
    fi
else
    echo -e "${YELLOW}⚠️  team-service/.env not found, creating from example...${NC}"
    if [ -f "team-service/.env.example" ]; then
        cp team-service/.env.example team-service/.env
        # Update database credentials
        sed -i 's/^DB_USERNAME=.*$/DB_USERNAME=root/' team-service/.env
        sed -i 's/^DB_PASSWORD=.*$/DB_PASSWORD=rootpassword/' team-service/.env
        sed -i 's/^DB_HOST=.*$/DB_HOST=team-db/' team-service/.env
        sed -i 's/^DB_DATABASE=.*$/DB_DATABASE=team_db/' team-service/.env
        echo -e "${GREEN}✅ Created team-service/.env with correct credentials${NC}"
        echo ""
    fi
fi
...
if [ -f "team-service/.env" ]; then
    docker cp team-service/.env ${SERVICE_NAME}:/var/www/html/.env 2>/dev/null || true
    ...
    echo -e "${YELLOW}⚠️  team-service/.env not found on host, using .env.example from container${NC}"
    docker-compose exec -T $SERVICE_NAME bash -c "if [ ! -f .env ]; then cp .env.example .env 2>/dev/null || touch .env; fi" || true
fi
echo ""
...
if [ $migration_exit_code -eq 0 ]; then
    ...
    if echo "$migration_output" | grep -qi "access denied\|1045"; then
        ...
        echo -e "  Run: ${YELLOW}docker-compose exec team-db mysql -uroot -prootpassword${NC}"
        echo -e "  Then execute:"
        echo -e "  ${YELLOW}ALTER USER 'user'@'%' IDENTIFIED BY 'rootpassword';${NC}"
        echo -e "  ${YELLOW}GRANT ALL PRIVILEGES ON team_db.* TO 'user'@'%';${NC}"
...
# Final Summary
echo -e "${BLUE}========================================${NC}"
echo -e "${GREEN}✅ Team Service setup completed successfully!${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
echo -e "${GREEN}Service is available at:${NC}"
echo -e "  - Direct: ${BLUE}http://localhost:8003${NC}"
echo -e "  - Health Check: ${BLUE}http://localhost:8003/api/health${NC}"
echo ""
echo -e "${GREEN}Next steps:${NC}"
echo -e "  1. Verify service is running: ${YELLOW}docker-compose ps team-service${NC}"
echo -e "  2. Check service logs: ${YELLOW}docker-compose logs -f team-service${NC}"
echo -e "  3. Test health endpoint: ${YELLOW}curl http://localhost:8003/api/health${NC}"
echo -e "  4. Test teams endpoint: ${YELLOW}curl http://localhost:8003/api/teams${NC}"
echo ""
echo -e "${YELLOW}Note: Team Service depends on Tournament Service.${NC}"
echo -e "${YELLOW}Make sure Tournament Service is set up and running first.${NC}"
echo ""
