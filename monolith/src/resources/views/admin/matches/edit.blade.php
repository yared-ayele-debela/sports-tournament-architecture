...
<div class="p-6">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-600 mt-1">Update match information</p>
            </div>
    ...
    <div class="bg-white shadow rounded-lg">
        <form action="{{ route('admin.matches.update', $match->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" 
                           id="current_minute" 
                           name="current_minute" 
                           value="{{ old('current_minute', $match->current_minute) }}"
                           min="0"
                           max="120"
    ...
    <div class="mt-6 bg-gray-50 rounded-lg p-4">
        <h3 class="text-sm font-medium text-gray-900 mb-2">Match Information</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
                <span class="text-gray-500">Current Home Team:</span>
                <span class="text-gray-900 font-medium">{{ $match->homeTeam->name ?? 'Not assigned' }}</span>
            </div>
            <div>
                <span class="text-gray-500">Current Away Team:</span>
                <span class="text-gray-900 font-medium">{{ $match->awayTeam->name ?? 'Not assigned' }}</span>
            </div>
            <div>
                <span class="text-gray-500">Current Tournament:</span>
                <span class="text-gray-900 font-medium">{{ $match->tournament->name ?? 'Not assigned' }}</span>
            </div>
            <div>
                <span class="text-gray-500">Current Venue:</span>
                <span class="text-gray-900 font-medium">{{ $match->venue->name ?? 'Not assigned' }}</span>
            </div>
            <div>
                <span class="text-gray-500">Current Status:</span>
                <span class="text-gray-900 font-medium">{{ ucfirst(str_replace('_', ' ', $match->status)) }}</span>
            </div>
            <div>
            ...
            <div>
                ...
                <span class="text-gray-900 font-medium">{{ $match->created_at->format('M d, Y H:i') }}</span>
            </div>
            <div>
                <span class="text-gray-500">Last Updated:</span>
                <span class="text-gray-900 font-medium">{{ $match->updated_at->format('M d, Y H:i') }}</span>
            </div>
        </div>
    </div>
</div>
