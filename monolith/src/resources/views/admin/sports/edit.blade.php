...
<div class="max-w-8xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Edit Sport</h1>
    ...
    <!-- Form -->
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <form action="{{ route('admin.sports.update', $sport->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <textarea id="description" 
                              ...
                              placeholder="Enter sport description (optional)">{{ old('description', $sport->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
    ...
    <div class="mt-6 bg-gray-50 rounded-lg p-4">
        <h3 class="text-sm font-medium text-gray-900 mb-2">Sport Information</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
                <span class="text-gray-500">Created:</span>
                <span class="text-gray-900 font-medium">{{ $sport->created_at->format('M d, Y H:i') }}</span>
            </div>
            <div>
                <span class="text-gray-500">Last Updated:</span>
                <span class="text-gray-900 font-medium">{{ $sport->updated_at->format('M d, Y H:i') }}</span>
            </div>
            <div>
                <span class="text-gray-500">Status:</span>
                <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {{ $sport->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
