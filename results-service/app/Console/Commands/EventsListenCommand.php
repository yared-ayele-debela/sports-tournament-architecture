 ...
 *
 * Run event listeners as a daemon process for Results Service
 *
 * This command listens for:
 * - sports.match.completed - Main event that triggers standings calculation
 * - sports.tournament.status.changed - Triggers full recalculation if needed
 */
class EventsListenCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Listen for events from Redis Pub/Sub channels (Results Service)';
    ...
    {
        $this->info('Starting Results Service event listener daemon...');
        $this->info('Service: ' . config('events.service.name', 'results-service'));
    ...
    protected function getChannels(): array
    {
        ...
        // Use single default channel from config
    ...
    protected function initializeHandlers(array $handlerConfigs): void
    {
        foreach ($handlerConfigs as $eventType => $handlers) {
            // $eventType is the event type (e.g., 'sports.match.completed')
