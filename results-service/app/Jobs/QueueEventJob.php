...
class QueueEventJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            // Get the queue name - Laravel sets this when the job is dispatched
            $queueName = $this->queue ?? 'default';
            
            Log::info('Processing queue event', [
            ...
            // Dispatch ProcessEventJob to handle the actual event processing
            // Use the same queue to ensure it's processed by the same worker
            $processJob = new ProcessEventJob($this->event);
            $processJob->onQueue($queueName);
            $processJob->onConnection($this->connection ?? config('queue.default'));
            dispatch($processJob);
            
            Log::info('ProcessEventJob dispatched', [
                'queue' => $queueName,
