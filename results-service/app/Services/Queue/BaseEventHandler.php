...
abstract class BaseEventHandler implements EventHandlerInterface
{
     ...
     * Uses processed_events table for idempotency
     *
     * @param string $eventId Event ID
     * @return bool
     */
    ...
    {
        try {
            return \Illuminate\Support\Facades\DB::table('processed_events')
                ->where('event_id', $eventId)
                ->exists();
        } catch (Exception $e) {
            Log::warning('Failed to check event processing status in database', [
                'event_id' => $eventId,
                'error' => $e->getMessage(),
                'service' => $this->serviceName,
            ]);
     ...
     * Stores in processed_events table for idempotency
     *
     * @param string $eventId Event ID
    ...
    {
        try {
            \Illuminate\Support\Facades\DB::table('processed_events')->insert([
                'event_id' => $eventId,
                'event_type' => $data['event_type'] ?? $this->getPayloadData(['payload' => []], 'event_type', 'unknown'),
                'processed_at' => Carbon::now(),
            ]);
        } catch (Exception $e) {
            Log::warning('Failed to mark event as processed in database', [
                'event_id' => $eventId,
                'error' => $e->getMessage(),
                'service' => $this->serviceName,
            ]);
    ...
    /**
     * Check if event has already been processed (database-based idempotency)
     *
     * @param string $eventId Event ID
     * @return bool
     */
    ...
    {
        return $this->isEventProcessed($eventId) || $this->isEventProcessedInDatabase($eventId);
    }

    /**
     * Mark event as processed (database-based idempotency)
     *
     * @param string $eventId Event ID
     * @param string|null $eventType Event type
     * @return void
     */
    protected function markAsProcessed(string $eventId, ?string $eventType = null): void
    {
        $this->markEventProcessed($eventId, $eventType ? ['event_type' => $eventType] : []);
        $this->markEventProcessedInDatabase($eventId, $eventType ? ['event_type' => $eventType] : []);
    }
