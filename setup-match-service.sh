...
# Script to setup Match Service
# This script automates the complete setup process for the match-service
# Usage: ./setup-match-service.sh
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Match Service Setup Script${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
# Step 3: Verify and fix match-service .env file
echo -e "${YELLOW}🔍 Checking match-service .env file...${NC}"
if [ -f "match-service/.env" ]; then
    # Check if DB_USERNAME is set correctly
    if grep -q "^DB_USERNAME=user" match-service/.env 2>/dev/null; then
        echo -e "${YELLOW}⚠️  Found DB_USERNAME=user in match-service/.env, updating to root...${NC}"
        # Update to use root user
        sed -i 's/^DB_USERNAME=user$/DB_USERNAME=root/' match-service/.env
        sed -i 's/^DB_PASSWORD=.*$/DB_PASSWORD=rootpassword/' match-service/.env
        echo -e "${GREEN}✅ Updated match-service/.env to use root credentials${NC}"
        echo ""
    elif grep -q "^DB_USERNAME=root" match-service/.env 2>/dev/null; then
        echo -e "${GREEN}✅ match-service/.env is configured correctly${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚠️  DB_USERNAME not found in match-service/.env, ensuring it's set...${NC}"
        # Ensure DB_USERNAME is set
        if ! grep -q "^DB_USERNAME=" match-service/.env 2>/dev/null; then
            echo "DB_USERNAME=root" >> match-service/.env
        fi
        if ! grep -q "^DB_PASSWORD=" match-service/.env 2>/dev/null; then
            echo "DB_PASSWORD=rootpassword" >> match-service/.env
        fi
        echo -e "${GREEN}✅ Added database credentials to match-service/.env${NC}"
        echo ""
    fi
else
    echo -e "${YELLOW}⚠️  match-service/.env not found, creating from example...${NC}"
    if [ -f "match-service/.env.example" ]; then
        cp match-service/.env.example match-service/.env
        # Update database credentials
        sed -i 's/^DB_USERNAME=.*$/DB_USERNAME=root/' match-service/.env
        sed -i 's/^DB_PASSWORD=.*$/DB_PASSWORD=rootpassword/' match-service/.env
        sed -i 's/^DB_HOST=.*$/DB_HOST=match-db/' match-service/.env
        sed -i 's/^DB_DATABASE=.*$/DB_DATABASE=match_db/' match-service/.env
        echo -e "${GREEN}✅ Created match-service/.env with correct credentials${NC}"
        echo ""
    fi
fi
...
if [ -f "match-service/.env" ]; then
    docker cp match-service/.env ${SERVICE_NAME}:/var/www/html/.env 2>/dev/null || true
    ...
    echo -e "${YELLOW}⚠️  match-service/.env not found on host, using .env.example from container${NC}"
    docker-compose exec -T $SERVICE_NAME bash -c "if [ ! -f .env ]; then cp .env.example .env 2>/dev/null || touch .env; fi" || true
fi
echo ""
...
if [ $migration_exit_code -eq 0 ]; then
    ...
    if echo "$migration_output" | grep -qi "access denied\|1045"; then
        ...
        echo -e "  Run: ${YELLOW}docker-compose exec match-db mysql -uroot -prootpassword${NC}"
        echo -e "  Then execute:"
        echo -e "  ${YELLOW}ALTER USER 'user'@'%' IDENTIFIED BY 'rootpassword';${NC}"
        echo -e "  ${YELLOW}GRANT ALL PRIVILEGES ON match_db.* TO 'user'@'%';${NC}"
...
# Final Summary
echo -e "${BLUE}========================================${NC}"
echo -e "${GREEN}✅ Match Service setup completed successfully!${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
echo -e "${GREEN}Service is available at:${NC}"
echo -e "  - Direct: ${BLUE}http://localhost:8004${NC}"
echo -e "  - Health Check: ${BLUE}http://localhost:8004/api/health${NC}"
echo ""
echo -e "${GREEN}Next steps:${NC}"
echo -e "  1. Verify service is running: ${YELLOW}docker-compose ps match-service${NC}"
echo -e "  2. Check service logs: ${YELLOW}docker-compose logs -f match-service${NC}"
echo -e "  3. Test health endpoint: ${YELLOW}curl http://localhost:8004/api/health${NC}"
echo -e "  4. Test matches endpoint: ${YELLOW}curl http://localhost:8004/api/matches${NC}"
echo ""
echo -e "${YELLOW}Note: Match Service depends on Auth, Tournament, and Team Services.${NC}"
echo -e "${YELLOW}Make sure those services are set up and running first.${NC}"
echo ""