...
# Script to setup Results Service
# This script automates the complete setup process for the results-service
# Usage: ./setup-results-service.sh
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Results Service Setup Script${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
if [ -f "results-service/.env" ]; then
    ...
    if grep -q "^DB_USERNAME=user" results-service/.env 2>/dev/null; then
        echo -e "${YELLOW}⚠️  Found DB_USERNAME=user in results-service/.env, updating to root...${NC}"
        # Update to use root user
        sed -i 's/^DB_USERNAME=user$/DB_USERNAME=root/' results-service/.env
        sed -i 's/^DB_PASSWORD=.*$/DB_PASSWORD=rootpassword/' results-service/.env
        echo -e "${GREEN}✅ Updated results-service/.env to use root credentials${NC}"
        echo ""
    elif grep -q "^DB_USERNAME=root" results-service/.env 2>/dev/null; then
        echo -e "${GREEN}✅ results-service/.env is configured correctly${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚠️  DB_USERNAME not found in results-service/.env, ensuring it's set...${NC}"
        # Ensure DB_USERNAME is set
        if ! grep -q "^DB_USERNAME=" results-service/.env 2>/dev/null; then
            echo "DB_USERNAME=root" >> results-service/.env
        fi
        if ! grep -q "^DB_PASSWORD=" results-service/.env 2>/dev/null; then
            echo "DB_PASSWORD=rootpassword" >> results-service/.env
        fi
        echo -e "${GREEN}✅ Added database credentials to results-service/.env${NC}"
        echo ""
    fi
else
    echo -e "${YELLOW}⚠️  results-service/.env not found, creating from example...${NC}"
    if [ -f "results-service/.env.example" ]; then
        cp results-service/.env.example results-service/.env
        # Update database credentials
        sed -i 's/^DB_USERNAME=.*$/DB_USERNAME=root/' results-service/.env
        sed -i 's/^DB_PASSWORD=.*$/DB_PASSWORD=rootpassword/' results-service/.env
        sed -i 's/^DB_HOST=.*$/DB_HOST=results-db/' results-service/.env
        sed -i 's/^DB_DATABASE=.*$/DB_DATABASE=results_db/' results-service/.env
        echo -e "${GREEN}✅ Created results-service/.env with correct credentials${NC}"
        echo ""
    fi
fi
...
if [ $migration_exit_code -eq 0 ]; then
    ...
    if echo "$migration_output" | grep -qi "access denied\|1045"; then
        ...
        echo -e "  Run: ${YELLOW}docker-compose exec results-db mysql -uroot -prootpassword${NC}"
        echo -e "  Then execute:"
        echo -e "  ${YELLOW}ALTER USER 'user'@'%' IDENTIFIED BY 'rootpassword';${NC}"
        echo -e "  ${YELLOW}GRANT ALL PRIVILEGES ON results_db.* TO 'user'@'%';${NC}"
...
# Final Summary
echo -e "${BLUE}========================================${NC}"
echo -e "${GREEN}✅ Results Service setup completed successfully!${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
echo -e "${GREEN}Service is available at:${NC}"
echo -e "  - Direct: ${BLUE}http://localhost:8005${NC}"
echo -e "  - Health Check: ${BLUE}http://localhost:8005/api/health${NC}"
echo ""
echo -e "${GREEN}Next steps:${NC}"
echo -e "  1. Verify service is running: ${YELLOW}docker-compose ps results-service${NC}"
echo -e "  2. Check service logs: ${YELLOW}docker-compose logs -f results-service${NC}"
echo -e "  3. Test health endpoint: ${YELLOW}curl http://localhost:8005/api/health${NC}"
echo -e "  4. Test standings endpoint: ${YELLOW}curl http://localhost:8005/api/tournaments/1/standings${NC}"
echo ""
echo -e "${YELLOW}Note: Results Service depends on Tournament, Match, and Team Services.${NC}"
