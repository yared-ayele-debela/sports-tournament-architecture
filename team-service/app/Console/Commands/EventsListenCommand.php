...
class EventsListenCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        $this->info('Service: ' . config('events.service.name', 'team-service'));
    ...
    protected function initializeHandlers(array $handlerConfigs): void
    {
        foreach ($handlerConfigs as $eventType => $handlers) {
            // Ensure $eventType is a string (event type name)
            // Ensure $handlers is an array of handler classes
            if (!is_string($eventType) || !is_array($handlers)) {
                $this->warn("Invalid handler configuration for event: " . var_export($eventType, true));
                continue;
            }

            foreach ($handlers as $handlerClass) {
                if (!is_string($handlerClass)) {
                    $this->warn("Handler class must be a string, got: " . gettype($handlerClass));
                    continue;
                }
                ...
                try {
                    ...
                    $this->info("Loaded handler: {$handlerClass}");
